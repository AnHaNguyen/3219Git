function drawLineGraph(data){
	...
	data.forEach(function(d) {
		...
		// Add high commit number to temps array
		commitNums.push(+d.totalNum);
	...
	// Calculate the range of the commit number data
	var yExtent = d3.extent(commitNums);
	...
	var avgCommitNumY = yScale(d3.mean(commitNums));

	// Path generator function for our data
	var pathGenerator = d3.svg.line()
		.x(function(d, i) { return xScale(dates[i]); })
		.y(function(d, i) { return yScale(commitNums[i]); });

	// Series container element
	var series = chart.append('g');

	// Add the commit number series path to the chart
	series.append('path')
	...
	function tooltipFormatter(date, commitNum) {
		var dateFormat = d3.time.format('%d %b %Y');
		return dateFormat(date) + '<br><b>' + commitNum;
	}
	...
	backRect.on('mousemove', function() {
		...
		// Get the i-th date value and commit number value.
		var _date = dates[i],
		    _commitNum = commitNums[i];

		// Update the position of the activePoint element
		activePoint.attr({
			cx: xScale(_date),
			cy: yScale(_commitNum)
		});

		// Update tooltip content
		tt.html(tooltipFormatter(_date, _commitNum));
